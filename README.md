...
Clone this repo, run `npm i` from top directory in your OS terminal to install the dependencies. Create a `.env` file in `/src/user` directory and populate it with database connection string (example below):
...
```
To send a GraphQL query, make a `POST` request with `Content-Type` header set to `application/json` and include a `JSON` object with the property `query` with the GraphQL query string as a value in the body of the request.
...
```
User records, i.e., nodes, in the output **are sorted in ascending order based on the time user records were updated** (`updatedAt` field in the schema) to introduce some consistency for pagination.
...
When you hit the last page the `hasNextPage` Boolean flag will turn `false` telling you when you can stop fetching. 
